...
# code -> (prefix, name). Only £ $ € are in Glancyr, so the rest are set as
# their ISO code, which is normal practice in those markets.
CURRENCIES = {
    "USD": ("$", "US dollar"),
    "GBP": ("\u00a3", "Pound sterling"),
    "EUR": ("\u20ac", "Euro"),
...
# used when only a region is known, for older proposals
CURRENCY = {"US": "$", "UK": "\u00a3"}
REGION_DEFAULT = {"US": "USD", "UK": "GBP"}


def symbol_for(code_or_region):
    """Accepts a currency code, or a region for older proposals."""
    key = (code_or_region or "USD").upper()
...
def money(value, currency="USD"):
    ...
    if value is None:
        return ""
    try:
        return "%s%s" % (symbol_for(currency), "{:,}".format(int(value)))
    except (TypeError, ValueError):
        return str(value)


def currency_of(data):
    """The proposal's currency, falling back to its region."""
    meta = data.get("meta", {}) if isinstance(data, dict) else {}
    code = (meta.get("currency") or "").upper()
    if code in CURRENCIES:
        return code
    return REGION_DEFAULT.get(meta.get("region", "US"), "USD")


def check_milestones(page12, region="US", currency=None):
    ...
    """
    cur = currency or REGION_DEFAULT.get(region, "USD")

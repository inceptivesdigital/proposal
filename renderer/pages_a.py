...
# --------------------------------------------------------------- page 2
# The client supplies this page finished, so it is used exactly as designed.
# The only thing drawn on top is the client's own name in the closing sentence,
# because the artwork carries the previous client's name.
# the slot in the artwork, measured on the supplied design (2480px = 595.2pt)
# measured on the supplied design: 2480px wide = 595.2pt, 3508px = 841.92pt
PX, PY = 595.2/2480, 841.92/3508
P2 = dict(x=141*PX, first_baseline=841.92 - 1258*PY, lead=66.8*PY,
          size=12.30, maxw=(1815-141)*PX,
          text="We don't just write code or design interfaces \u2014 we understand "
               "how products fit into people's lives. Whether launching an MVP "
               "against the clock, redesigning for a better experience, or scaling "
               "to millions of users, we combine creativity, strategy, and "
               "technical excellence. We're not just your development team; we're "
               "your technology partner, committed to seeing {client} thrive long "
               "after launch.")


def page2(c, d):
    """Company Overview. The design is used exactly as supplied; only the closing
    paragraph is redrawn, because the artwork carries a previous client's name."""
    plate(c, 2)
    name = as_text(d.get("meta", {}).get("client_company")).strip() or "your business"
    text = P2["text"].format(client=name)
    lines = wrap(text, "G-Light", P2["size"], P2["maxw"])
    draw_lines(c, lines, P2["x"], P2["first_baseline"], P2["lead"], "G-Light",
               P2["size"], (0.09, 0.10, 0.12))
    map_field("meta.client_company", P2["x"], P2["first_baseline"], P2["maxw"],
              len(lines)*P2["lead"], P2["size"], "G-Light",
...
P4 = dict(title_x=41.3, title_y=[651.5, 606.5], title_size=47.24,
          ...
          card_x=[36.4, 213.0, 389.3], card_w=165.0,
          card_y=88.4, card_h=213.1,
          rule=dict(w=24.5, h=1.2, y=216.5, dx=15.9),
          rule_colour=[(0.22, 0.44, 0.98), (0.59, 0.76, 0.41), (0.23, 0.44, 0.97)])
RULE_GREY = (198/255, 203/255, 219/255)


P4_CLEAN = os.path.join(PLATES, "page4_clean.jpg")
P4_SLOT = (306, 168, 252, 356)      # x, top-origin y, w, h


def page4(c, d):
    p4 = d.get("page4", {})
    # the artwork carries a phone mockup; a clean plate exists for when the
    # client's own screen goes there instead, or nothing does
    if p4.get("clean_background") and os.path.exists(P4_CLEAN):
        c.drawImage(P4_CLEAN, 0, 0, width=W, height=H)
        shot = (d.get("_screens") or {}).get(p4.get("screen", ""))
        if shot or p4.get("screen"):
            x, top, w, h = P4_SLOT
            device_with_screen(c, shot, x, H-top-h, w, h, r=20)
    else:
        plate(c, 4)
    p = d["page4"]
    ...
    # a short row is reported once, by the review, not twice
    # one size for all three cards, so they read as a set
    tsize = P4["title2_size"]
    while tsize > 10 and any(len(wrap(as_text(c["title"]), "G-Reg", tsize,
                                      P4["title2_maxw"])) > 2 for c in cards):
        tsize -= 0.3
    bsize = P4["body_size"]
    while bsize > 8.5 and any(len(wrap(as_text(c.get("body", "")), "G-XLight",
                                       bsize, P4["body_maxw"])) > 4
                              for c in cards):
        bsize -= 0.25
    for i, card in enumerate(cards):
        # the cards are vector now, so only the ones with content are drawn
        soft_panel(c, P4["card_x"][i], P4["card_y"], P4["card_w"], P4["card_h"],
                   12, (1, 1, 1), 0.10)
        tile(c, P4["icon_cx"][i], P4["icon_cy"], 17.0)
        c.setFillColorRGB(*P4["rule_colour"][i % 3])
        c.rect(P4["card_x"][i] + P4["rule"]["dx"], P4["rule"]["y"],
               P4["rule"]["w"], P4["rule"]["h"], stroke=0, fill=1)
        draw_icon(c, P4["icon_cx"][i], P4["icon_cy"], P4["icon_box"],
                  ic(card.get("icon", "ic_spark")))
        ts = tsize
        tl = wrap(as_text(card["title"]), "G-Reg", ts, P4["title2_maxw"])[:2]
        c.setFillColorRGB(*INK)
        c.setFont("G-Reg", ts)
        ...
        draw_lines(c, wrap(as_text(card.get("body", "")), "G-XLight", bsize,
                           P4["body_maxw"])[:4],
                   P4["body_x"][i], P4["body_y"], P4["body_lead"], "G-XLight",
                   bsize, path="page4.cards.%d.body" % i, maxw=P4["body_maxw"])
...
P8 = dict(eyebrow=(36.2, 719.2, 14.0), headline=(36.2, [684.2, 655.2], 30.49),
          ...
          device=(292, 268, 286, 322))


GRID_BOTTOM = {0: 356.0, 1: 356.0, 2: 92.0, 3: 92.0}   # floor of each card


def _grid_items(c, items, x, y0, maxw, page, idx, path=None):
    ...
    for j, (b, t) in enumerate(pairs):
        ...
        if y0 - (height + h) < floor:
            ...
            break
        if path:
            map_field("%s.items.%d.text" % (path, j), x, y0 - height, maxw,
                      h, size, "G-Light")

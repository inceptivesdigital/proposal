...
def page3(c, d):
    ...
    while len(wrap(p["one_liner"], "G-Med", size, P3["head_maxw"])) > 3 and size > 14:
        size -= 0.3
    draw_lines(c, wrap(p["one_liner"], "G-Med", size, P3["head_maxw"]),
               P3["head_x"], P3["head_y"], P3["head_lead"], "G-Med", size)
    lines = []
...
def page4(c, d):
    ...
    draw_lines(c, wrap(p["description"], "G-Light", P4["desc_size"],
                       P4["desc_maxw"])[:7],
               P4["desc_x"], P4["desc_y"], P4["desc_lead"], "G-Light",
               P4["desc_size"])
...
def _core_grid(c, page, spec, screens):
    ...
    headline(c, P5["headline"][0], P5["headline"][1], spec["headline"],
             P5["headline"][2])
    for i, card in enumerate(spec["cards"][:4]):
        ...
        flow(c, [(it.get("bullet", False), it["text"]) for it in card["items"]],
             P5["bx"][i], P5["by"][i], 9.5, P5["bw"][i], 12.0, 12.0)
...
def _core_list(c, page, spec):
    ...
    headline(c, P6["headline"][0], P6["headline"][1], spec["headline"],
             P6["headline"][2])
    for i, card in enumerate(spec["cards"][:4]):
...
def _core_device(c, page, spec, screens, cfg):
    ...
    eyebrow(c, cfg["eyebrow"][0], cfg["eyebrow"][1], spec["eyebrow"],
            cfg["eyebrow"][2])
    headline(c, cfg["headline"][0], cfg["headline"][1], spec["headline"],
             cfg["headline"][2])

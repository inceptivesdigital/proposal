...
_PATCHES = [None]          # the current document, so plate() can see repairs


def set_patch_source(data):
    _PATCHES[0] = data


def plate(canvas, page):
    """Draw the page artwork, with any repairs the user asked for applied."""
    path = os.path.join(PLATES, "page%d.jpg" % page)
    data = _PATCHES[0]
    if data:
        from .patch import for_page, plate_path
        marks = for_page(data, page)
        if marks:
            path = plate_path(page, marks)
    canvas.drawImage(path, 0, 0, width=W, height=H)
...
def dotted(c, x, y, parts, font, size, colour=GREY, sep="  \u00b7  "):
    """Glancyr has no middle dot, so that glyph alone is set in Helvetica."""
...
# ordinary prose, used to convert a column width into a character budget
_SAMPLE = ("Buyers browse listings on an interactive map and request a viewing "
           "in a few taps while agents keep real time visibility of every offer")


def capacity(font, size, maxw, lines=1):
    """How many characters of ordinary prose fit in this box."""
    try:
        per = sw(_SAMPLE, font, size) / len(_SAMPLE)
        return max(int(maxw / per) * max(lines, 1), 1)
    except Exception:                                         # noqa: BLE001
        return 0


def map_field(path, x, y, w, h, size=10, font="G-Light", align="left",
              lines=None, label=None):
    """Record a click target for the editor overlay. y is the text baseline."""
    if not path:
        return
    if lines is None:
        lines = max(int(round(h / max(size * 1.25, 1))), 1)
    TEXTMAP.append({"page": _PAGE[0], "path": path, "x": round(x, 1),
                    ...
                    "lines": lines, "label": label or _label_for(path),
                    "limit": capacity(font, size, w, lines)})


_LABELS = {
    "page1.title": "Cover headline", "page1.description": "Cover description",
    "page3.one_liner": "App one-liner", "page3.description": "App description",
    "page3.surfaces_heading": "Surfaces heading",
    "page4.one_liner": "Differentiator line", "page4.description":
    "Differentiator description", "page9.headline": "Marketing headline",
    "page9.description": "Marketing description",
}
...
def _label_for(path):
    for prefix, label in _LABELS.items():
        if path.startswith(prefix):
            return label
    bits = [b for b in path.split(".") if not b.isdigit()]
    tail = bits[-1] if bits else path
    section = bits[-2] if len(bits) > 1 else ""
    pretty = tail.replace("_", " ").capitalize()
    if section and not section.startswith("page"):
        return "%s %s" % (section.rstrip("s").capitalize(), pretty.lower())
    return pretty


def qa_note(page, kind, detail):
    """page is ignored: the renderer always knows which page it is drawing.
    Callers used to pass a mix of indexes, numbers and literals."""
    # _PAGE holds the index of the page currently being drawn, so the number
    # reported is always the number printed on the PDF
    QA.append({"page": _PAGE[0], "kind": kind, "detail": detail})
...
def device_frame(c, x, y, w, h, r=14):
    """A light bezel. The dark one read as a black slab on a pale page."""
    c.saveState()
    c.setFillColorRGB(0.34, 0.40, 0.52, alpha=0.16)
    c.roundRect(x+2, y-7, w, h, r, stroke=0, fill=1)
    c.setFillAlpha(1)
